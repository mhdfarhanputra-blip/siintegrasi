  ...
  for (let idx = 9; idx < raw.length; idx++) {
    ...
    for (let col = 0; col < 16; col++) {
      ...
      if (val != null && String(val).trim()) {
        uraianParts.push(String(val).trim())
      }
    }
    ...
    if (row[7] != null && !isNaN(Number(row[7]))) {
      kodeAkun = String(Math.round(Number(row[7])))
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (me.role !== 'Admin') {
      return NextResponse.json({ error: 'Hanya Admin yang dapat mengupload data realisasi' }, { status: 403 })
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json({ error: 'Ukuran file melebihi batas 10 MB' }, { status: 400 })
    }
    ...
    if (data.length === 0) {
      return NextResponse.json({ error: 'Tidak ditemukan data realisasi dalam file. Pastikan format sesuai Laporan FA Detail (16 Segmen).' }, { status: 400 })
    }

    // Simpan ke file JSON statis — path di-resolve secara aman
    const baseDir = resolve(process.cwd(), 'src', 'lib')
    const jsonPath = join(baseDir, 'realisasi-data.json')
